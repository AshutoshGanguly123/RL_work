...
class DuelingLinearDeepQNetwork(nn.Module):
    def __init__(self, ALPHA, n_actions, name, input_dims, chkpt_dir = '/Users/ashutoshganguly/Desktop/RL_work/lunar_lander/dueling_ckpts'):
        ...
        self.to(self.device)
        self.dir = chkpt_dir
        self.checkpoint_file = os.path.join(self.dir,name+'dueling_dqn')
...
class Agent(object):
    def __init__(self, gamma, epsilon, alpha, n_actions, input_dims, mem_size, batch_size, eps_min =0.1, eps_dec=5e-7, replace=1000, chkpt_dir= '/Users/ashutoshganguly/Desktop/RL_work/lunar_lander/dueling_ckpts' ):
        self.gamma = gamma
...
# Ensure tensors have batch dimensions
        state = T.tensor(state, dtype=T.float32).to(self.q_eval.device)
        new_state = T.tensor(new_state, dtype=T.float32).to(self.q_eval.device)
        action = T.tensor(action).to(self.q_eval.device)
        reward = T.tensor(reward, dtype=T.float32).to(self.q_eval.device)
